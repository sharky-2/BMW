...
function shuffleAnimation(event) {
    ...
    // Select all .word inside #container-shuffel
    const allWords = document.querySelectorAll("#container-shuffel .word");

    // Filter only those that are descendants of the current hovered .item (target)
    const words = Array.from(allWords).filter(word => target.contains(word));
